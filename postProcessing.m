...
if (fid > 0)
    ...
    if ~settings.VLLen
        ...
        load 'C:\Sara\School\ASEN-6080StatOD2\Project1\inputs\trackingResults0~360spll25dll2'
        settings.VLLen = 1;%enable vector tracking
    end
    ...
    if ~settings.VLLen
        ...
        load 'C:\Sara\School\ASEN-6080StatOD2\Project1\inputs\navSolutions0~360sPRN2224141819'
        disp('start vector tracking')
        %start vector tracking
        trackingvdeep2(fid, channel,trackResults,navSolutions,eph,activeChnList,svTimeTable, settings);
        fclose all;
        return;     
    end
